...
app.controller('MestreEntidade', [
    '$scope', '$http', '$location', '$anchorScroll', '$timeout', '$modal', function ($scope, $http, $location, $anchorScroll, $timeout, $modal) {
        ...
        $scope.LoadBancos = function () {
            ...
            $http.post("../../SBI/ObterBanco/", $scope.Filter)
                .success(function (data) {
        ...
        $scope.ObterPorId = function (value) {
            ...
            $http.post("../../SBI/ObterPorId", { id: value })
                .success(function (data) {
                    $scope.showGrid = false;
                    $scope.showManut = true;
                    Metronic.unblockUI();
                    $scope.entidade = data;
                    $scope.entidade.banco_id = $scope.Filter.banco_id;
                }).error(function (data) {
        ...
        $scope.Salvar = function () {
            ...
            $http.post("../../SBI/Salvar", $scope.entidade)
                .success(function (data) {
                    Metronic.unblockUI();
        ...
        //TODO: Carrega Demais
         ...
         var ControllerExcluir = function ($scope, $modalInstance, $modal, $http, $timeout, item) {
           ...
           $http.post("../../SBI/Excluir", { obj: $scope.entidade })
                .success(function (data) {
                   Metronic.unblockUI();
                   if (data.CDStatus === "OK") {
