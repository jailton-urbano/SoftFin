...
app.controller('MestreEntidade', [
    '$scope', '$http', '$location', '$anchorScroll', '$timeout', '$modal', function ($scope, $http, $location, $anchorScroll, $timeout, $modal) {
        ...
        $scope.data = Date.now();
        $scope.SIMNAO = [{ Value: false, Text: "Não" }, { Value: true, Text: "Sim" }];


        $scope.EditAccess = false;
        $http.post("../../RC/AcessoEdicao")
        ...
        $scope.filtro = {
            emaberto: true,
            dataInicial: moment($scope.data).add(-20, 'days').format(),
            dataFinal: moment($scope.data).add(20, 'days').format(),
            valorInicial: 0.00,
            valorFinal: 9999999.99
        };
        ...
        $scope.ObterTodos = function () {
            ...
            $http.post("../../RC/ObterTodos/", { pg: $scope.filtro })
            .success(function (data) {
                $scope.lista = data;
                Metronic.unblockUI();
            }).error(function (data) {
        ...
        $scope.Salvar = function () {
            ...
            $http.post("../../RC/Salvar", $scope.entidade)
            .success(function (data) {
                Metronic.unblockUI();
                ...
                if (data.CDStatus == "OK") {
                    ...
                    $scope.msgSalvar = "Salvar";
                    $scope.ObterPorId($scope.entidade.idnf);
        ...
        $scope.ObterPorId = function (value) {
            ...
            $http.post("../../RC/ObterPorId", { id: value })
            .success(function (data) {
        ...
        $scope.LoadObterTipoDocumento = function () {
            Metronic.blockUI({
                boxed: true
            });
            $http.post("../../RC/ObterTipoDocumento")
                .success(function (data) {
                    Metronic.unblockUI();
        ...
        $scope.LoadObterBanco = function () {
            Metronic.blockUI({
                boxed: true
            });
            $http.post("../../RC/ObterBanco")
                .success(function (data) {
                    Metronic.unblockUI();
        ...
        $scope.LoadObterBanco();
        $scope.LoadObterTipoDocumento();
        $scope.ObterTodos();
    }]);
...
var ControllerExcluir = function ($scope, $modalInstance, $modal, $http, $timeout, item) {
    ...
    $scope.ok = function () {
        ...
        $http.post("../../RC/Excluir", { obj: $scope.item })
        .success(function (data) {
            $scope.btnSalvar = "OK";
            if (data.CDStatus == "OK") {
