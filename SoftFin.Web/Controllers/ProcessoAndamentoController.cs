...
{
    public class ProcessoAndamentoController : BaseController
    {
        ...
        public JsonResult Obter(string codigoProcesso)
        {
            ...
            var retorno = new CallApi.GestorProcessos().ObterAtividadeExecucaoOutros(paramProcesso);
        ...
        public JsonResult ObterUsuarios()
        {
            ...
            var retorno = new CallApi.GestorProcessos().ObterUsuarios(param);

            return Json(retorno, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult DelegarAtividade(string novousuario, string codigoAtividadeExecucaoAtual)
        {
            var param = new GestorProcessos.Comum.Param.ParamProcesso();
            param.CodigoEmpresa = _estabobj.Empresa.codigo;
            param.CodigoUsuario = _usuario;

            var retorno = new CallApi.GestorProcessos().DelegarAtividade(param, novousuario, codigoAtividadeExecucaoAtual);

            return Json(retorno, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult CancelarProcesso(string motivo, string codigoProcessoAtual)
        {
            var param = new GestorProcessos.Comum.Param.ParamProcesso();
            param.CodigoEmpresa = _estabobj.Empresa.codigo;
            param.CodigoUsuario = _usuario;

            var retorno = new CallApi.GestorProcessos().CancelarProcesso(param, motivo, codigoProcessoAtual);
