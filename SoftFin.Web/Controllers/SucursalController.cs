...
namespace SoftFin.Web.Controllers
{
    public class SucursalController : BaseController
    {
        ...
        private List<Sucursal> Organiza(JqGridRequest request, List<Sucursal> objs)
        {

            return new SoftFin.Utils.UtilSoftFin.GenericSorter<Sucursal>().Sort(objs.AsQueryable(), request.SortingName, request.SortingOrder).ToList();
        }
        ...
        public ActionResult Listas(JqGridRequest request)
        {
            ...
            int totalRecords = 0;
            var objs = new Sucursal().ObterTodos();
        ...
        [HttpPost]
        public JsonResult Salvar(Sucursal entidade)
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult Excluir(Sucursal entidade)
        {
            try
            {
                if (entidade.ObterPorId(entidade.id) != null)
        ...
        public JsonResult ObterId(int id)
        {
            try
            {
                var obj = new Sucursal().ObterPorId(id);
                return Json(new
                {
                    CDStatus = "OK",
                    entidade = (obj == null) ? new Sucursal() : obj
                }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
