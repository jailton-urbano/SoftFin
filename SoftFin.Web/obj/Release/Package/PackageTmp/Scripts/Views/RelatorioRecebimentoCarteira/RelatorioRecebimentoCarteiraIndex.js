...
app.controller('MestreEntidade', [
    ...
    function ($scope, $sce, $http, $location, $anchorScroll, $timeout, $modal) {
        //Variaveis
        $scope.reverse = false;
        $scope.CleanSum = function () {
            $scope.sumCarteira = 0;
            $scope.sumRealizado = 0;
            $scope.sumValoresAberto = 0;
        };
        $scope.CleanSum();
        ...
        $scope.Search = function () {
            ...
            if ($scope.filter.excel == false) {
                ...
                $http.post("../../RelatorioRecebimentoCarteira/ObterRelatorio",
                    {
        ...
        $scope.SumResult = function () {
            $scope.CleanSum();
            for (var i = 0; i < $scope.lista.length; i++) {
                $scope.sumCarteira += $scope.lista[i].CarteiraValor;
                $scope.sumRealizado += $scope.lista[i].RealizadoValor;
                var teste = parseFloat($scope.lista[i].ValoresAberto);

                if (!isNaN(teste)) {
                    $scope.sumValoresAberto += parseFloat($scope.lista[i].ValoresAberto);
                }
            };
        };
        ...
        $scope.DownloadExcel = function () {
            ...
            mapForm.id = "stmtForm";
            mapForm.method = "POST";
            mapForm.action = "../../RelatorioRecebimentoCarteira/ObterRelatorio";
